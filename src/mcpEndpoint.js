...
const INSTRUCTIONS =
	...
	+ 'the board") - answer it from that, and do not open the components one by one. Both '
	+ 'carry structure.outline, the board\'s map: each section with its title or the heading '
	+ 'read from inside it and what it holds, clusters, connections - so an untitled section '
	+ 'is never "no title" or "empty" in your answer. '
...
const BRIDGE_TOOLS = [
	...
	{
		name: 'read_board',
		description: 'Read what is currently on the connected board: every component with its id, type, label, position, size, its text content (including structured content like checklist items or kanban cards), and what it can be converted into (convertTo). This is the whole-board answer - use it to understand what exists before adding to it, and to answer questions about the board itself, without opening the components one by one. It also carries structure: the board\'s map derived from positions and connectors (no AI) - sections with their title or, when the title is blank, the heading read from the text placed inside them, their children in reading order; connector edges (from/to, direction, label); clusters of components placed near each other; grouped sets; note colours; and outline, the same facts as indented text - so answer "what is on this board" from structure.outline plus the per-component text, and never treat an untitled section as empty or unknown. text is shortened per component, and textTruncated: true marks one that was cut; when the user then asks about that particular component, read_board_component gives you it in full. On a large board, search_board narrows to what you actually need instead. Base what you render on the content that is actually there, and when you could not read a component, say so rather than inventing the rest.',
		inputSchema: {
			type: 'object',
			properties: {
	...
	},
	{
		...
		inputSchema: {
			...
			properties: {
				...
				componentType: { type: 'string', description: 'Restrict to one kind of component, like grep\'s --type. A friendly name ("mindmap", "kanban", "wireframe", "flowchart", "document", "moodboard") or a registry key ("MF_MindMap_ID"); matched loosely, including diagram-frame kinds ("sequence diagram", "cloud architecture").' },
